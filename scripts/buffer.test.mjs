...
import { loadQueueState, markRendered, pendingRender, publishable } from "./queue.mjs";
...
test("publishable returns rendered videos that have not posted, oldest first", () => {
  const state = {
    posted: ["d01-a"],
    rendered: [entry("d01-a"), entry("d01-b"), entry("d01-c")],
  };
  assert.deepEqual(
    publishable(state).map((item) => item.id),
    ["d01-b", "d01-c"],
  );
});

test("publishable is empty when the buffer is drained", () => {
  assert.deepEqual(publishable({ posted: ["d01-a"], rendered: [entry("d01-a")] }), []);
  assert.deepEqual(publishable({ posted: [] }), []);
});
